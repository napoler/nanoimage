...
use std::path::Path;
use nanoimage_core::{OptimizerConfig, ImageFormat};

#[test]
fn test_image_format_detection_jpeg() {
    assert_eq!(ImageFormat::from_path(Path::new("test.jpg")), ImageFormat::Jpeg);
    assert_eq!(ImageFormat::from_path(Path::new("test.jpeg")), ImageFormat::Jpeg);
}
...
fn test_image_format_detection_gif() {
    assert_eq!(ImageFormat::from_path(Path::new("test.gif")), ImageFormat::Gif);
}

#[test]
fn test_image_format_detection_bmp() {
    assert_eq!(ImageFormat::from_path(Path::new("test.bmp")), ImageFormat::Bmp);
}

#[test]
fn test_image_format_detection_svg() {
    assert_eq!(ImageFormat::from_path(Path::new("test.svg")), ImageFormat::Svg);
}

#[test]
fn test_image_format_detection_unknown() {
    assert_eq!(ImageFormat::from_path(Path::new("test.txt")), ImageFormat::Unknown);
}
...
fn test_optimizer_config_lossy_mode() {
    use nanoimage_core::CompressionMode;
    let config = nanoimage_core::OptimizerConfig { mode: CompressionMode::Lossy, ..Default::default() };
...
fn test_optimizer_config_custom_quality() {
    ...
    let config = nanoimage_core::OptimizerConfig { quality: Quality { lossy: 95, lossless: 90 }, ..Default::default() };
...
fn test_optimizer_config_max_dimensions() {
    let config = nanoimage_core::OptimizerConfig { max_width: Some(1920), max_height: Some(1080), ..Default::default() };
...
fn test_optimizer_config_output_format() {
    ...
    let config = nanoimage_core::OptimizerConfig { format: OutputFormat::WebP, ..Default::default() };
    assert_eq!(config.format.as_str(), "webp");
}

#[test]
fn test_optimizer_config_overwrite() {
    let config = nanoimage_core::OptimizerConfig { overwrite: true, ..Default::default() };
